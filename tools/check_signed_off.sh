...
function print_help
{
  ...
  echo "'Signed-off-by: NAME EMAIL', where NAME and EMAIL must"
...
then
  ...
  required_signed_off_by_line="Signed-off-by: $author_name $author_email"
  ...
  if echo "$actual_signed_off_by_line" | grep -q -v '^Signed-off-by:'
  then
    echo -e "\e[1;33mSigned-off-by message is incorrect. The following line should be at the end of the $commit_hash commit's message: '$required_signed_off_by_line'. \e[0m"
    exit 1
  fi
